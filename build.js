...
function build() {
  ...
  var proc = cp.spawn(process.execPath, args, {
...
function getElectronModuleVersion() {
  ...
  return new Promise(function(resolve) {
    var cmd = electronPath + ' -e "process.stdout.write(process.versions.modules)"';
    ...
    cp.exec(cmd, opts, (err, stdout, stderr) => {
      ...
      if (stderr.length) {
        console.error('Failed to get Electron\'s module version: ' + stderr.toString());
        process.exit(1);
      }
      if (!stdout.length) {
        console.error('Failed to get Electron\'s module version');
